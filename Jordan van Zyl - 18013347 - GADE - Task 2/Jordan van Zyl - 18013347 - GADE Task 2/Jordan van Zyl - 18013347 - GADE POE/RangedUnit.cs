...
namespace Jordan_van_Zyl___18013347___GADE_POE
{
    class RangedUnit : Unit
    {
        ...
        public RangedUnit(string name, int pos_X, int pos_Y, int health, int maxHealth, int speed, int attack, int attackRange, string team, string symbol, bool isAttacking) : base(name, pos_X, pos_Y, health, maxHealth, speed, attack, attackRange, team, symbol, isAttacking)
        {
            ...
            for (int i = 0; i < enemy.Length; i++)
            {
                if (enemy[i] != null)
                {
                    ...
                    if (enemyType == "RangedUnit")
                    {
                        ...
                        if (r_Unit.Team != this.team)
                        {
                            enemy_X = r_Unit.Pos_X;
                            enemy_Y = r_Unit.Pos_Y;

                            if (enemy_X != this.pos_X && enemy_Y != this.pos_Y)
                            {
                    ...
                    else
                    {
                        ...
                        if (m_Unit.Team != this.team)
                        {
                            enemy_X = m_Unit.Pos_X;
                            enemy_Y = m_Unit.Pos_Y;
